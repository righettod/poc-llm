...
kanban
    ...
    step02[Testing]    
        step02-task01[In CI, use a skill to review the findings of a SAST scan to remove false positive.]
        step02-task02[In CI, use a skill to analyze the code from user input perspective to identify flaws reachable by a user.]
        step02-task03[In CI, use a skill to update the project documentation based on the real state of the code base.]     
        step02-task04[In CI, in a sandbox to handle compromised release, try to use the latest release of every library, if something fails then ask to the code assistant to propose a PR with the code to update.]               
```
